import requests
import certifi
import json
from bs4 import BeautifulSoup
from urllib import parse

class City():

    def get_city_url(self):
        url='http://dateandtime.info/ru/city.php?id='
        city=input("Выберите город: ")
        cities = {
        'Абиджан': '2293538',
        'Манаус': '3663517',
        'Абу-Грейб': '100077',
        'Мандалай': '1311874',
        'Абу-Даби': '292968',
        'Манила': '1701668',
        'Агадир': '2561668',
        'Мапуту': '1040652',
        'Адана': '325363',
        'Мар-дель-Плата': '3430863',
        'Аддис-Абеба': '344979',
        'Маракай': '3632998',
        'Аделаида': '2078025',
        'Маракайбо': '3633009',
        'Аден': '415189',
        'Мариуполь': '701822',
        'Аккра': '2306104',
        'Марракеш': '2542997',
        'Александрия': '361058',
        'Марсель': '2995469',
        'Алеппо': '170063',
        'Маскат': '287286',
        'Алжир': '2507480',
        'Матола': '1039854',
        'Алматы': '1526384',
        'Матурин': '3778045',
        'Амман': '250441',
        'Мбужи-Майи': '209228',
        'Амстердам': '2759794',
        'Мванза': '152224',
        'Анкара': '323786',
        'Медан': '1214520',
        'Анталия': '323777',
        'Медельин': '3674962',
        'Антананариву': '1070940',
        'Медина': '109223',
        'Антверпен': '2803138',
        'Мекка': '104515',
        'Арекипа': '3947322',
        'Мекнес': '2542715',
        'Асмэра': '343300',
        'Мельбурн': '2158177',
        'Асунсьон': '3439389',
        'Мемфис': '4641239',
        'Асьют': '359783',
        'Мендоса': '3844421',
        'Афины': '264371',
        'Мехико': '3530597',
        'Ахваз': '144448',
        'Мешхед': '124665',
        'Ахмадабад': '1279233',
        'Милан': '3173435',
        'Ашхабад': '162183',
        'Минск': '625144',
        'Багдад': '98182',
        'Миссиссога': '6075357',
        'Баколод': '1729564',
        'Могадишо': '53654',
        'Баку': '587084',
        'Моламьяйн': '1308465',
        'Балтимор': '4347778',
        'Момбаса': '186301',
        'Бамако': '2460596',
        'Монреаль': '6077243',
        'Бангалор': '1277333',
        'Монровия': '2274895',
        'Банги': '2389853',
        'Монтевидео': '3441575',
        'Бангкок': '1609350',
        'Монтеррей': '3995465',
        'Бандунг': '1650357',
        'Москва': '524901',
        'Баркисимето': '3648522',
        'Мосул': '99072',
        'Барнаул': '1510853',
        'Музаффарабад': '1169607',
        'Барранкилья': '3689147',
        'Мултан': '1169825',
        'Барселона': '3128760',
        'Мумбаи': '1275339',
        'Барселона': '3648559',
        'Мурсия': '2513416',
        'Басра': '99532',
        'Мюнхен': '2867714',
        'Бейра': '1052373',
        'Нагоя': '1856057',
        'Бейрут': '276781',
        'Нагпур': '1262180',
        'Бекаси': '1649378',
        'Найроби': '184745',
        'Белград': '792680',
        'Наманган': '1513157',
        'Белен': '3405870',
        'Нампхо': '1873757',
        'Белу-Оризонти': '3470127',
        'Нанкин': '1799962',
        'Бенгази': '88319',
        'Наньчун': '1800146',
        'Бенин-Сити': '2347283',
        'Нджамена': '2427123',
        'Бенони': '1020098',
        'Неаполь': '3172394',
        'Берлин': '2950159',
        'Нейпьидо': '6611854',
        'Бирмингем': '2655603',
        'Несауалькойотль': '3530589',
        'Бишкек': '1528675',
        'Ниамей': '2440485',
        'Блантайр': '931755',
        'Нижний Новгород': '520555',
        'Богота': '3688689',
        'Николаев': '700569',
        'Бостон': '4930956',
        'Новосибирск': '1496747',
        'Браззавиль': '2260535',
        'Нуакшот': '2377450',
        'Бразилиа': '3469058',
        'Нью-Йорк': '5128581',
        'Брамптон': '5907364',
        'Нюрнберг': '2861650',
        'Братислава': '3060972',
        'Одесса': '698740',
        'Бремен': '2944388',
        'Окленд': '2193733',
        'Брисбен': '2174003',
        'Омдурман': '365137',
        'Бристоль': '2654675',
        'Омск': '1496153',
        'Брюссель': '2800866',
        'Оран': '2485926',
        'Буаке': '2290956',
        'Осака': '1853909',
        'Будапешт': '3054643',
        'Осло': '3143244',
        'Букараманга': '3688465',
        'Остин': '4671654',
        'Булавайо': '894701',
        'Оттава': '6094817',
        'Бумердес': '2474141',
        'Палембанг': '1633070',
        'Бурса': '750269',
        'Палермо': '2523920',
        'Бухарест': '683506',
        'Париж': '2988507',
        'Буэнос-Айрес': '3435910',
        'Патна': '1260086',
        'Валенсия': '3625549',
        'Пекин': '1816670',
        'Валенсия': '2509954',
        'Перейра': '3672486',
        'Ванкувер': '6173331',
        'Пермь': '511196',
        'Варшава': '756135',
        'Перт': '2063523',
        'Вашингтон': '4140963',
        'Петалинг-Джая': '1735158',
        'Вена': '2761369',
        'Пешавар': '1168197',
        'Вильнюс': '593116',
        'Пикин': '2246678',
        'Виннипег': '6183235',
        'Питермарицбург': '965301',
        'Владивосток': '2013348',
        'Пномпень': '1821306',
        'Волгоград': '472757',
        'Познань': '3088171',
        'Воронеж': '472045',
        'Порт-о-Пренс': '3718426',
        'Вроцлав': '3081368',
        'Порт-Саид': '358619',
        'Гаага': '2747373',
        'Порт-Судан': '377039',
        'Гавана': '3553478',
        'Порт-Харкорт': '2324774',
        'Газа': '281133',
        'Порт-Элизабет': '964420',
        'Газиантеп': '314830',
        'Порту-Алегри': '3452925',
        'Гайранвала': '1177662',
        'Прага': '3067696',
        'Гамбург': '2911298',
        'Претория': '964137',
        'Гамильтон': '5969782',
        'Приштина': '786714',
        'Ганновер': '2910831',
        'Пуна': '1259229',
        'Гаосюн': '1673820',
        'Пусан': '1838524',
        'Гаруа': '2231320',
        'Пуэбла-де-Сарагоса': '3521081',
        'Гвадалахара': '4005539',
        'Пуэнт-Нуар': '2255414',
        'Гватемала': '3598132',
        'Пуэнте-Альто': '3875024',
        'Гданьск': '3099434',
        'Пхеньян': '1871859',
        'Генерал-Сантос': '1713022',
        'Рабат': '2538475',
        'Генуя': '3176219',
        'Равалпинди': '1166993',
        'Гётеборг': '2711537',
        'Раджшахи': '1185128',
        'Гиза': '360995',
        'Ресифи': '3390760',
        'Глазго': '2648579',
        'Рига': '456172',
        'Голд-Кост': '2165087',
        'Рим': '3169070',
        'Гомель': '627907',
        'Рио-де-Жанейро': '3451190',
        'Гонконг': '1819729',
        'Росарио': '3838583',
        'Гояния': '3462377',
        'Ростов-на-Дону': '501175',
        'Гуанчжоу': '1809858',
        'Роттердам': '2747891',
        'Гуаякиль': '3657509',
        'Сайтама': '6940394',
        'Давао': '1715348',
        'Салвадор': '3450554',
        'Дакар': '2253354',
        'Сале': '2537763',
        'Дакка': '1185241',
        'Сальта': '3838233',
        'Даллас': '4684888',
        'Самара': '499099',
        'Дамаск': '170654',
        'Сан-Антонио': '4726206',
        'Даммам': '110336',
        'Сан-Диего': '5391811',
        'Дананг': '1583992',
        'Сан-Мигель-де-Тукуман': '3836873',
        'Дар-эс-Салам': '160263',
        'Сан-Паулу': '3448439',
        'Дели': '1273294',
        'Сан-Педро-Сула': '3601782',
        'Денвер': '5419384',
        'Сан-Сальвадор': '3583361',
        'Детройт': '4990729',
        'Сан-Франциско': '5391959',
        'Джайпур': '1269515',
        'Сан-Хосе': '5392171',
        'Джакарта': '1642911',
        'Сан-Хуан': '4568127',
        'Джибути': '223817',
        'Сана': '71137',
        'Джидда': '105343',
        'Санкт-Петербург': '498817',
        'Джохор-Бару': '1732752',
        'Санта-Крус-де-ла-Сиерра': '3904906',
        'Джэксонвилл': '4160021',
        'Санта-Марта': '3668605',
        'Днепр': '709930',
        'Санта-Фе': '3836277',
        'Донецк': '709717',
        'Санто-Доминго': '3492908',
        'Дортмунд': '2935517',
        'Сантьяго': '3871336',
        'Дрезден': '2935022',
        'Сантьяго-де-Куба': '3536729',
        'Дуала': '2232593',
        'Сантьяго-де-лос-Кабальерос': '3492914',
        'Дубай': '292223',
        'Сапопан': '3979770',
        'Дублин': '2964574',
        'Саппоро': '2128295',
        'Дуйсбург': '2934691',
        'Сарагоса': '3104324',
        'Дунгуань': '1812545',
        'Сараево': '3191281',
        'Дурбан': '1007311',
        'Саратов': '498677',
        'Душанбе': '1221874',
        'Себу': '1717512',
        'Дюссельдорф': '2934246',
        'Севастополь': '694423',
        'Екатеринбург': '1486209',
        'Севилья': '2510911',
        'Ереван': '616052',
        'Сендай': '2111149',
        'Загреб': '3186886',
        'Сеул': '1835848',
        'Замбоанга': '1679432',
        'Сиань': '1790630',
        'Запорожье': '687700',
        'Сидней': '2147714',
        'Ибаге': '3680656',
        'Сингапур': '1880252',
        'Ибадан': '2339354',
        'Сиэтл': '5809844',
        'Иерусалим': '281184',
        'Скопье': '785842',
        'Ижевск': '554840',
        'София': '727011',
        'Измир': '311046',
        'Стамбул': '745044',
        'Икитос': '3696183',
        'Стокгольм': '2673730',
        'Индаур': '1269743',
        'Сувон': '1835553',
        'Индианаполис': '4259418',
        'Сулеймания': '98463',
        'Инчхон': '1843564',
        'Сурабая': '1625822',
        'Иокогама': '1848354',
        'Сурат': '1255364',
        'Ипох': '1734634',
        'Суэц': '359796',
        'Иркутск': '2023469',
        'Сьюдад-Гуаяна': '3645528',
        'Исфахан': '418863',
        'Сьюдад-Хуарес': '4013708',
        'Йоханнесбург': '993800',
        'Табук': '101628',
        'Кабул': '1138958',
        'Тагиг': '1684308',
        'Кавасаки': '1859642',
        'Таиз': '70225',
        'Кадуна': '2335727',
        'Тайбэй': '1668341',
        'Казань': '551487',
        'Тайнань': '1668355',
        'Каир': '360630',
        'Тайчжун': '1668399',
        'Калгари': '5913490',
        'Тангеранг': '1625084',
        'Кали': '3687925',
        'Танжер': '2530335',
        'Калькутта': '1275004',
        'Ташкент': '1512569',
        'Кальяо': '3946083',
        'Тбилиси': '611717',
        'Кампала': '232422',
        'Тебесса': '2477461',
        'Кананга': '214481',
        'Тебриз': '113646',
        'Кано': '2335204',
        'Тегеран': '112931',
        'Канпур': '1267995',
        'Тегусигальпа': '3600949',
        'Караганда': '609655',
        'Тембиза': '949880',
        'Карай': '128747',
        'Тихуана': '3981609',
        'Каракас': '3646738',
        'Токио': '1850147',
        'Карачи': '1174872',
        'Тольятти': '482283',
        'Картахена': '3687238',
        'Торонто': '6167865',
        'Касабланка': '2553604',
        'Триполи': '2210247',
        'Катманду': '1283240',
        'Трухильо': '3691175',
        'Кванджу': '1841811',
        'Туба': '2244322',
        'Квебек': '6325494',
        'Тулуза': '2972315',
        'Кветта': '1167528',
        'Тунис': '2464470',
        'Кейптаун': '3369157',
        'Турин': '3165524',
        'Кёльн': '2886242',
        'Тэгу': '1835329',
        'Кербела': '94824',
        'Тэджон': '1835235',
        'Кесон-Сити': '1692192',
        'Тяньцзинь': '1792947',
        'Кигали': '202061',
        'Уагадугу': '2357048',
        'Киев': '703448',
        'Улан-Батор': '2028462',
        'Кильмес': '3429652',
        'Ульяновск': '479123',
        'Кингстон': '3489854',
        'Уфа': '479561',
        'Киншаса': '2314302',
        'Ухань': '1791247',
        'Киото': '1857910',
        'Фейсалабад': '1179400',
        'Киркук': '94787',
        'Фес': '2548885',
        'Кисангани': '212730',
        'Филадельфия': '4560349',
        'Кито': '3652462',
        'Финикс': '5308655',
        'Кишинёв': '618426',
        'Форт-Уэрт': '4691930',
        'Кланг': '1732905',
        'Форталеза': '3399415',
        'Кобе': '1859171',
        'Франкфурт-на-Майне': '2925533',
        'Колвези': '922773',
        'Фритаун': '2409306',
        'Коломбо': '1248991',
        'Фукуока': '1863967',
        'Колумбус': '4509177',
        'Хабаровск': '2022890',
        'Конакри': '2422465',
        'Хайдарабад': '1269843',
        'Константина': '2501152',
        'Хайдарабад': '1176734',
        'Конья': '306571',
        'Хайфон': '1581298',
        'Копенгаген': '2618425',
        'Хама': '170017',
        'Кордова': '3860259',
        'Хамхын': '1877449',
        'Кота-Бару': '1736376',
        'Ханой': '1581130',
        'Котону': '2394819',
        'Ханчжоу': '1808926',
        'Кочабамба': '3919968',
        'Хараре': '890299',
        'Коян': '1842485',
        'Харбин': '2037013',
        'Краков': '3094802',
        'Харгейсе': '57289',
        'Краснодар': '542420',
        'Хартум': '379252',
        'Красноярск': '1502026',
        'Харьков': '706483',
        'Кривой Рог': '703845',
        'Хельсинки': '658225',
        'Куала-Лумпур': '1735161',
        'Хиросима': '1862415',
        'Кукута': '3685533',
        'Ходейда': '79415',
        'Кум': '119208',
        'Хомс': '169577',
        'Кумаси': '2298890',
        'Хошимин': '1566083',
        'Куритиба': '3464975',
        'Хьюстон': '4699066',
        'Куссери': '2229798',
        'Цзюлун': '1819609',
        'Кучинг': '1735634',
        'Челябинск': '1508291',
        'Кхулна': '1336135',
        'Ченнаи': '1264527',
        'Ла-Пас': '3911925',
        'Чикаго': '4887398',
        'Ла-Плата': '3432043',
        'Чиклайо': '3698350',
        'Лагос': '2332459',
        'Читтагонг': '1205733',
        'Лакхнау': '1264733',
        'Чунцин': '1814906',
        'Лахор': '1172451',
        'Чэнду': '1815286',
        'Лейпциг': '2879139',
        'Шанхай': '1796236',
        'Леон': '3998655',
        'Шарджа': '292672',
        'Либревиль': '2399697',
        'Шарлотт': '4460243',
        'Ливерпуль': '2644210',
        'Шах-Алам': '1732903',
        'Лидс': '2644688',
        'Шеффилд': '2638077',
        'Ликаси': '922741',
        'Шираз': '115019',
        'Лилонгве': '927967',
        'Штутгарт': '2825297',
        'Лима': '3936456',
        'Шымкент': '1518980',
        'Лион': '2996944',
        'Шэньчжэнь': '1795565',
        'Лиссабон': '2267057',
        'Шэньян': '2034937',
        'Лодзь': '3093133',
        'Эдинбург': '2650225',
        'Ломе': '2365267',
        'Эдмонтон': '5946768',
        'Лондон': '2643743',
        'Эз-Зарка': '250090',
        'Лос-Анджелес': '5368361',
        'Экатепек-де-Морелос': '3529612',
        'Луанда': '2240449',
        'Эль-Мансура': '360761',
        'Лубумбаши': '922704',
        'Эль-Махалла-эль-Кубра': '360829',
        'Луганск': '702658',
        'Эль-Пасо': '5520993',
        'Луксор': '360502',
        'Эн-Наджаф': '98860',
        'Лусака': '909137',
        'Эр-Рияд': '108410',
        'Львов': '702550',
        'Эрбиль': '95446',
        'Мадрид': '3117735',
        'Эссен': '2928810',
        'Макао': '1821274',
        'Эт-Таиф': '107968',
        'Макассар': '1622786',
        'Янгон': '1298824',
        'Малага': '2514256',
        'Ярославль': '468902',
        'Манагуа': '3617763',
        'Яунде': '2220957'}

        for c in cities:
            if city.lower() in c.lower():
                city_url = url + cities[c]
                self.city_id=cities[c]
                print(c, city_url)
                return(city_url)
            
    def get_wiki_coordinates(self, city):
        ua={
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_8_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.1453.116 Safari/537.36'
        }
        url='https://ru.wikipedia.org/wiki/'+city+'#/maplink/0'
        print(url)
        result=requests.get(url, headers=ua)
        self.html=BeautifulSoup(result.text, features="html.parser") #, 'lxml'
        #print(self.html)
        coord_start=str(self.html).find('"wgCoordinates":{')+len('"wgCoordinates":{')
        coordinates=str(self.html)[coord_start:coord_start+30]
        #print(coordinates)
        lat = coordinates[6:coordinates.find(',')]
        long = (coordinates[coordinates.find(',')+7:coordinates.find(',')+7+15]).replace('},"','')
        #print('!!!!', lat, 'and!!!!!!!!!!', long)
        return(lat, long)
    
    def get_coordinates(self):
        ua={
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_8_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.1453.116 Safari/537.36'
        }
        try:
            url=self.get_city_url()
            print(url)
            result=requests.get(url, headers=ua)            
            #VOZMOZHNA OSHIBKA
            self.html=BeautifulSoup(result.text, features="html.parser") #, 'lxml')
            coordinates=self.html.find_all('div', {'class':'level1'})
            coordinates = str(coordinates[4]).replace('-','')
            self.coordinates=coordinates[coordinates.find("</a>:")+6:coordinates.find("</a>:")+6+30]
            #print(self.coordinates)
            lat = self.coordinates[:8]
            long = self.coordinates[self.coordinates.find(',')+2:self.coordinates.find(',')+2+15]
            try:
                lat_gr = int(lat[0:2])
                lat_min = int(lat[3:5])
                lat_sec = int(lat[6:8])
            except:
                try:
                    lat_gr = int(lat[0:3])
                    lat_min = int(lat[4:6])
                    lat_sec = int(lat[7:9])
                except:
                    lat_gr = int(lat[0:1])
                    lat_min = int(lat[2:4])
                    lat_sec = int(lat[5:7])                    
            try:
                long_gr = int(long[0:2])
                long_min = int(long[3:5])
                long_sec = int(long[6:8])
            except:
                try:
                    long_gr = int(long[0:3])
                    long_min = int(long[4:6])
                    long_sec = int(long[7:9])
                except:
                    long_gr = int(long[0:1])
                    long_min = int(long[2:4])
                    long_sec = int(long[5:6])
                    
            #print(lat_gr, lat_min, lat_sec,"\t",long_gr, long_min, long_sec)
            
            self.long = str((long_gr*60*60+long_min*60+long_sec)/60/60)
            self.lat = str((lat_gr*60*60+lat_min*60+lat_sec)/60/60)
        except:
            city = input("Упс... ничего не нашлось. Попробуйте указать полное название города: ")
            coord=self.get_wiki_coordinates(city)            
            self.lat, self.long = coord
        print(self.lat, self.long)

class Weather():

    def __init__(self, lat, long):
        self.url='https://api.weather.yandex.ru/v1/forecast?lat='+lat+'&lon='+long+'&extra=true'

    def send_request(self):
        ua_key={
            'X-Yandex-API-Key': '2414fc48-5065-42db-a861-f8f62c38bb16' #'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_8_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.1453.116 Safari/537.36',
            }
        self.result=requests.get(self.url, headers=ua_key).text

    def get_json(self):
        self.json_str=self.result
        
    @staticmethod
    def get_prec(self):
        pass

    @staticmethod
    def get_cloudness(self):
        pass

    @staticmethod
    def get_prec_strength(self):
        pass
    
    @staticmethod
    def get_condition(cond_code):
        conditions = {
        'clear':'Ясно',
        'partly-cloudy':'Малооблачно',
        'cloudy':'Облачно с прояснениями',
        'overcast':'Пасмурно',
        'partly-cloudy-and-light-rain':'Небольшой дождь',
        'partly-cloudy-and-rain':'Дождь',
        'overcast-and-rain':'Сильный дождь',
        'overcast-thunderstorms-with-rain': 'Сильный дождь, гроза',
        'cloudy-and-light-rain': 'Небольшой дождь',
        'overcast-and-light-rain':'Небольшой дождь',
        'cloudy-and-rain':'Дождь',
        'overcast-and-wet-snow':'Дождь со снегом',
        'partly-cloudy-and-light-snow':'Небольшой снег',
        'partly-cloudy-and-snow':'Снег',
        'overcast-and-snow':'Снегопад',
        'cloudy-and-light-snow':'Небольшой снег',
        'overcast-and-light-snow':'Небольшой снег',
        'cloudy-and-snow':'Снег',
        }
        return(conditions[cond_code])

    @staticmethod
    def get_wind_direct(wind_direct_code):
         directions = {
        'nw':'северо-западный',
        'n':'северный',
        'ne':'северо-восточный',
        'e':'восточный',
        'se':'юго-восточный',
        's':'южный',
        'sw':'юго-западный',
        'w': 'западный',
        'с': 'штиль'
        }
         return(directions[wind_direct_code])
                
    def json_parce_now(self):
        self.parsed_string = json.loads(self.json_str)
        print('\t\tСейчас: \n')
        print(self.get_condition(self.parsed_string['fact']['condition']))       
        print('Температура воздуха', self.parsed_string['fact']['temp'], '*C')
        print('Ощущается как', self.parsed_string['fact']['feels_like'], '*C')
        try:
            print('Температура воды',    self.parsed_string['fact']['temp_water'])
        except:
            pass
        print('Ветер', str(self.get_wind_direct(self.parsed_string['fact']['wind_dir']))+', скорость', self.parsed_string['fact']['wind_speed'],'м/с, порывы до', self.parsed_string['fact']['wind_gust'],'м/с')
        print('Атмосферное давление', self.parsed_string['fact']['pressure_mm'], 'мм. рт. ст.')
        print('Относительная влажность', str(self.parsed_string['fact']['humidity'])+'%')


    def json_parce_fore(self):
        self.parsed_string = json.loads(self.json_str)
        part=['morning','day','evening','night']
        head=['утро','день','вечер','ночь']
        print('\t\tПрогноз:')
        for p, h in zip(part, head):
            print('\n\t\t',h)
            print(self.get_condition(self.parsed_string['forecasts'][0]['parts'][p]['condition']))
            #print(self.parsed_string['forecasts'][0]['parts'])
            print('Температура воздуха', self.parsed_string['forecasts'][0]['parts'][p]['temp_avg'], '*C')
            print('Ощущается как', self.parsed_string['forecasts'][0]['parts'][p]['feels_like'], '*C')
            try:
                print('Температура воды',    self.parsed_string['forecasts'][0]['parts'][p]['temp_water'])
            except:
                pass
            print('Ветер', str(self.get_wind_direct(self.parsed_string['forecasts'][0]['parts'][p]['wind_dir']))+', скорость', self.parsed_string['forecasts'][0]['parts'][p]['wind_speed'],'м/с, порывы до', self.parsed_string['forecasts'][0]['parts'][p]['wind_gust'],'м/с')
            print('Атмосферное давление', self.parsed_string['forecasts'][0]['parts'][p]['pressure_mm'], 'мм. рт. ст.')
            print('Относительная влажность', str(self.parsed_string['forecasts'][0]['parts'][p]['humidity'])+'%')
       


if __name__ == '__main__':

    c=City()
    c.get_coordinates()
    long=c.long
    lat=c.lat

    w=Weather(lat,long)
    w.send_request()
    w.get_json()
    w.json_parce_now()
    print('\n\n')
    w.json_parce_fore()


    
                  
